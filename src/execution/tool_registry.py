...
class ToolResult:
    ...
    def __init__(self, success: bool, output: str, error: str = "", needs_approval: bool = False):
...
class ToolRegistry:
    ...
    async def execute(
        self, tool_name: str, params: dict, approval_callback: Callable = None
    ) -> ToolResult:
        """
        执行工具（带沙箱检查）
    ...
    async def _execute_l3(self, tool, params, approval_callback) -> ToolResult:
        ...
        if not approval_callback:
            return ToolResult(False, "", f"工具 {tool.name} 需要审批", needs_approval=True)
